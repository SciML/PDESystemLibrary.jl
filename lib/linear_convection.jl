...
function linear_convection(f, ps, name = :linear_convection)
    ...
    # PDE system
    lin_conv = PDESystem(eq, bcs, domains, [t, x], [u(t, x)], [v]; defaults = Dict(v => ps[1]),
...
function convection_diffusion(L, ps, name = :convection_diffusion)
    ...
    # PDE system
    convdiff = PDESystem(eq, bcs, domains, [t, z], [f(t, z)], [v]; defaults = Dict(k => ps[1], v => ps[2]), analytic_func = ref, metadata = tags,
                         name = name)
